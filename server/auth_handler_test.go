...
type hookTrackingAuthHandler struct {
	*InMemoryAuthHandler
...
func (h *hookTrackingAuthHandler) OnAuthSuccess(conn *Conn) error {
...
func (h *hookTrackingAuthHandler) OnAuthFailure(conn *Conn, err error) {
	h.onFailureCalled.Add(1)
}

func TestOnAuthSuccessCalled(t *testing.T) {
	handler := &hookTrackingAuthHandler{
		InMemoryAuthHandler: NewInMemoryAuthHandler(mysql.AUTH_NATIVE_PASSWORD),
	}
